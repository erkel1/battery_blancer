...
def startup_self_test(settings, stdscr):
    ...
    logging.info("Starting self-test: Validating config, connectivity, sensors, and balancer.")  # Log test start
    ...
    if alerts:
        startup_failed = True  # Mark startup as failed
        ...
        if y < stdscr.getmaxyx()[0]:  # Check if message fits
            try:
                stdscr.addstr(y, 0, "Self-Test Complete with Failures. Continuing with warnings.", curses.color_pair(2))  # Display in red
            except curses.error:
                logging.warning("addstr error for self-test failures.")  # Log display error
    else:
        if y < stdscr.getmaxyx()[0]:  # Check if message fits
            try:
                stdscr.addstr(y, 0, "Self-Test Complete. All OK.", curses.color_pair(4))  # Display in green
            except curses.error:
    ...
    return alerts  # Return failure alerts
