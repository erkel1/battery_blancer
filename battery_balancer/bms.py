...
#
# **Script Name:** bms.py
# **Version:** 1.4 (As of August 24, 2025) - Updated to support parallel batteries with multiple Modbus slaves on the same RS485 bus (multidrop configuration). Each parallel battery has its own Modbus slave address, but shares the same IP and port via TCP tunnel on the EDS4100. Supports configurable number of parallel batteries (e.g., 4 for 3s32p effective configuration with 96 temperature sensors). Temperature readings are aggregated across all slaves, grouped by series bank for medians, deviations, and alerts. TUI and web dashboard now show bank summaries (median, min, max, invalid count) instead of individual sensors for scalability. Added error handling for individual slave failures at startup and runtime, with connectivity tests per slave. Ensures no blank screen by displaying errors in TUI and logging.
...
# - Calibration: On first valid read (all sensors > valid_min across all slaves), computes overall median and offsets. Saves to 'offsets.txt'.
# - Anomalies Checked: Same as before, but alerts include battery ID, series bank, and local channel (e.g., "Battery 2 Bank 1 Local Ch 3: High temp").
# - **Voltage Monitoring & Balancing:** Same as before (ADS1115 for 3 series banks; voltages unchanged by parallels). Balances if difference > threshold or for heating (<10°C). Skips if alerts (including slave failures).
# - **Alerts & Notifications:** Logs to 'battery_monitor.log'. Activates alarm on issues, including slave connectivity. Throttled emails.
# - **Watchdog:** Pets during operations.
# - **User Interfaces:**
#   - **TUI:** Curses-based real-time display with ASCII art, summaries per bank (median/min/max/invalid temps), alerts, balancing, events, ASCII charts for voltages and overall median temp.
#   - **Web Dashboard:** HTTP on port 8080. Shows voltages, bank summaries, alerts, balancing. API for status/balance/history. Interactive Chart.js for time-series.
# - **Time-Series Logging:** RRDTool for voltages and overall median temp.
# - **Startup Self-Test:** Validates config, I2C/Modbus connectivity (per slave), initial reads (aggregated), balancer. Retries on failure.
# - **Error Handling:** Per-slave Modbus errors shown in alerts/TUI, fallback readings. No blank screen – errors displayed.
# - **Configuration:** From 'battery_monitor.ini'. Now includes number_of_parallel_batteries and modbus_slave_addresses.
# - **Logging:** Configurable level.
# - **Shutdown:** Cleans up on exit.
#
# **Key Features Explained for Non-Programmers:**
# - Guardian for battery pack, now supporting parallel batteries (e.g., 4 identical 3s8p in parallel for higher capacity).
# - Temperatures: Checks 96 thermometers (24 per battery). Groups by series position for analysis.
# - Voltages: Same as single, since parallel.
# - Heating/Balancing: Generates heat in cold, equalizes voltages.
# - Alerts: Includes which battery/slave failed.
# - Interfaces: Terminal shows summaries/charts; web shows interactive charts/summaries.
# - Startup: Tests each Modbus slave connection.
#
# **How It Works (Step-by-Step for Non-Programmers):**
# 1. Load settings, including parallel count and slave addresses.
# 2. Setup hardware, compute sensor groupings.
# 3. Self-Test: Check config, I2C, each Modbus slave, initial aggregated reads, balancing.
# 4. Main Loop:
#    - Read temps from each slave, aggregate, handle errors.
#    - Calibrate/apply offsets.
#    - Check temp issues per global channel (with battery/local info).
#    - Read voltages.
#    - Check voltage issues.
#    - Update RRD.
#    - Balance if needed (heating or diff).
#    - Fetch history.
#    - Update TUI/web with summaries.
#    - Log/email if issues.
#    - Pet watchdog.
#    - Sleep, repeat.
# 5. Balancing: Same.
...
# (Same as before, but "Read Temps" now "Read Temps from Each Slave, Aggregate", "Check Temp Issues" with battery info, self-test includes per-slave checks.)
#
# **Dependencies:** Same as before.
#
# **Installation Guide:** Same, but ensure RS485 wiring for multidrop (daisy-chain slaves).
#
# **Notes & Troubleshooting:**
# - Parallel Support: Ensure slave addresses unique, connected via RS485 to EDS4100 port 1.
# - If slave fails: Alerted, uses fallback readings (valid_min), continues.
# - TUI/Web: Summaries to handle many sensors (no individual display).
# - Offsets: Now for all 96.
# - Common Errors: Modbus - check addresses/wiring; add DEBUG for details.
# - Customization: Adjust parallel count/addresses in INI.
#
# **battery_monitor.ini Documentation (With Comments - Copy This to Your File):**
# (Updated below with new keys.)
#
# --------------------------------------------------------------------------------
...
web_data = {
    'voltages': [0.0] * 3,
    'bank_summaries': [{'median': 0.0, 'min': 0.0, 'max': 0.0, 'invalid': 0}] * 3,
    'alerts': [],
    'balancing': False,
...
def get_bank_for_channel(ch):
    """
    Find which series bank a global channel belongs to.
    Args:
        ch (int): Global channel number (1 to total_channels).
    Returns:
        int: Bank number (1 to 3) or None if invalid.
    """
...
def get_battery_and_local_ch(ch):
    """
    Find the battery ID and local channel for a global channel.
    Args:
        ch (int): Global channel (1 to total_channels).
    Returns:
...
def load_config():
    ...
    # Initialize alert states for total channels
...
def set_relay_connection(high, low, settings):
    try:
        ...
        logging.info(f"Relay setup completed for balancing from Bank {high} to Bank {low}")
            ...
            try:
                ...
                if y_test < stdscr.getmaxyx()[0]:
                    try:
                        ...
                        logging.warning(f"addstr error for Modbus Slave {addr} OK.")
            except Exception as e:
                ...
                if y_test < stdscr.getmaxyx()[0]:
                    try:
                        ...
                        logging.warning(f"addstr error for Modbus Slave {addr} failure.")
            y_test += 1
            stdscr.refresh()
        y = y_test
