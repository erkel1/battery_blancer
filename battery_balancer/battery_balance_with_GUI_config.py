...
try:
    bus = smbus.SMBus(config.getint('General', 'I2C_BUS'))  # Use bus specified in config
    GPIO.setmode(GPIO.BCM)
    GPIO.setup(config.getint('GPIO', 'DC_DC_RELAY_PIN'), GPIO.OUT)
    GPIO.setup(config.getint('GPIO', 'ALARM_RELAY_PIN'), GPIO.OUT, initial=GPIO.LOW)
...
def select_channel(channel):
    ...
    try:
        bus.write_byte(config.getint('I2C', 'PAHUB2_ADDR'), 1 << channel)
...
def config_vmeter():
    ...
    config_value = config.getint('ADS1115', 'CONFIG_CONTINUOUS') | config.getint('ADS1115', 'CONFIG_RATE_8') | config.getint('ADS1115', 'CONFIG_PGA_6144')
    try:
        bus.write_word_data(config.getint('I2C', 'VMETER_ADDR'), config.getint('ADS1115', 'REG_CONFIG'), config_value)
    ...
    for attempt in range(max_retries):
        try:
            ...
            for _ in range(num_samples):
                ...
                adc_samples.append(bus.read_word_data(config.getint('I2C', 'VMETER_ADDR'), config.getint('ADS1115', 'REG_CONVERSION')) & 0xFFFF)
...
def set_relay(high_cell, low_cell):
    ...
    try:
        ...
        bus.write_byte_data(config.getint('I2C', 'RELAY_ADDR'), 0x10, relay_state)
...
def control_dc_dc(enable):
    ...
    try:
        GPIO.output(config.getint('GPIO', 'DC_DC_RELAY_PIN'), GPIO.HIGH if enable else GPIO.LOW)
...
def send_email_alarm():
    ...
    msg = MIMEText(f'Warning: Cell voltage exceeded {config.getfloat("General", "ALARM_VOLTAGE_THRESHOLD")}V!')
...
def check_overvoltage_alarm(voltages):
    ...
    for i, voltage in enumerate(voltages):
        if voltage and voltage > config.getfloat('General', 'ALARM_VOLTAGE_THRESHOLD'):
            logging.warning(f"ALARM: Cell {i+1} voltage is {voltage:.2f}V, exceeding threshold!")
            try:
                GPIO.output(config.getint('GPIO', 'ALARM_RELAY_PIN'), GPIO.HIGH)
                send_email_alarm()
            except Exception as e:
                logging.error(f"Error during overvoltage alarm: {e}")
            return True
    try:
        GPIO.output(config.getint('GPIO', 'ALARM_RELAY_PIN'), GPIO.LOW)
    except Exception as e:
...
def balance_cells(stdscr):
    ...
    voltages = []
    adc_raws = []
    for cell in range(config.getint('General', 'NUM_CELLS')):
    ...
    if max_voltage - min_voltage > config.getfloat('General', 'BALANCE_THRESHOLD'):
        stdscr.addstr(10, 0, f"Balancing Cell {high_cell+1} to Cell {low_cell+1}...")
        set_relay(high_cell, low_cell)
        control_dc_dc(False)
        time.sleep(0.1)
        control_dc_dc(True)
        time.sleep(config.getint('General', 'BALANCE_TIME'))
        control_dc_dc(False)
        stdscr.addstr(10, 0, "Balancing completed. Waiting for stabilization...")
        time.sleep(config.getint('General', 'BALANCE_REST_PERIOD'))
